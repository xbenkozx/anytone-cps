...
void Device::readChannelData(){
    ...
    for(int i = 0; i < channel_id_list.size(); i++){
        ...
        int primary_data_address = offset + (
            (idx - (channel_data_block * block_channels)) * Anytone::Memory::Map()->ChannelDataOffset) + 
            (channel_data_block * Anytone::Memory::Map()->ChannelDataBlockOffset);
...
void Device::readTalkgroupData(){
    QByteArray set_list_data = readMemory(Anytone::Memory::Map()->TalkgroupSet, 0x4f0);
    ...
    int offset = Anytone::Memory::Map()->TalkgroupData;
    for(int i = 0; i < id_list.size(); i++){
        ...
        int addr = offset + (idx * Anytone::Memory::Map()->TalkgroupDataOffset);
        QByteArray tg_data = readMemory(addr, Anytone::Memory::Map()->TalkgroupDataLength);
        if(!is_alive) return;
        Anytone::Memory::talkgroups.at(idx)->decode(tg_data);
    }
}
void Device::readZoneData(){
    int zone_name_offset = Anytone::Memory::Map()->ZonesName;
    int zone_channel_offset = Anytone::Memory::Map()->ZoneChannels;
    int zone_a_channel_offset = Anytone::Memory::Map()->ZoneAChannel;
    int zone_b_channel_offset = Anytone::Memory::Map()->ZoneBChannel;
    int zone_hide_offset = Anytone::Memory::Map()->ZoneHide;

    QByteArray zone_set_data = readMemory(Anytone::Memory::Map()->ZoneSet, 0x20);
    ...
    for(int i = 0; i < id_list.size(); i++){
        ...
        // Name
        int addr = zone_name_offset + (idx * Anytone::Memory::Map()->ZoneDataOffset);
        QByteArray zone_name_data = readMemory(addr, Anytone::Memory::Map()->ZoneDataLength);
        if(!is_alive) return;
...
void Device::readRadioIdData(){
    QByteArray set_data = readMemory(Anytone::Memory::Map()->RadioIdSet, 0x20);
    std::vector<int> id_list;
    ...
    int offset = Anytone::Memory::Map()->RadioIdData;
    for(int i = 0; i < id_list.size(); i++){
        emit update2(i, id_list.size(), "Reading Scan List Data");

        int idx = id_list.at(i);
        if(idx >= Anytone::Memory::radioids.size()) continue;
        int addr = offset + (idx * Anytone::Memory::Map()->RadioIdDataOffset);
        QByteArray data = readMemory(addr, Anytone::Memory::Map()->RadioIdDataLength);
        if(!is_alive) return;
        Anytone::Memory::radioids.at(idx)->decode(data);
    }
}
void Device::readScanListData(){
    QByteArray set_data = readMemory(Anytone::Memory::Map()->ScanListSet, 0x20);
    std::vector<int> id_list;
    ...
    int offset = Anytone::Memory::Map()->ScanListData;
    for(int i = 0; i < id_list.size(); i++){
        emit update2(i, id_list.size(), "Reading Scan List Data");

        int idx = id_list.at(i);
        if(idx >= Anytone::Memory::scanlists.size()) continue;
        int addr = offset + (idx * Anytone::Memory::Map()->ScanListDataOffset);
        QByteArray scan_list_data = readMemory(addr, Anytone::Memory::Map()->ScanListDataLength);
        if(!is_alive) return;
...
void Device::readFmData(){
    if(Anytone::Memory::radio_model == Anytone::RadioModel::D878UVII_FW400){
        ...
        QByteArray fm_data = readMemory(0x3402000, 0x60);
        Anytone::Memory::fm_channels.at(100)->frequency = QString(fm_data.mid(0x0, 0x4).toHex()).toInt();
        Anytone::Memory::fm_channels.at(100)->name = QString(fm_data.mid(0x0, 0x4));
        Anytone::Memory::fm_channels.at(100)->name.remove(QChar('\0'));
        for(int byte_idx = 0; byte_idx < 16; byte_idx++){
            emit update2(byte_idx, 16, "Reading FM Data");
            uint8_t active_byte = fm_data.at(0x40 + byte_idx);
            uint8_t scan_byte = fm_data.at(0x50 + byte_idx);
            for(int bit_index = 0; bit_index < 8; bit_index++){
                int fm_index = (byte_idx * 8) + bit_index;
                bool is_active = Bit::test(active_byte, bit_index);
                if(!is_active) continue;
                ...
                Anytone::FM *fm = Anytone::Memory::fm_channels.at(fm_index);
                fm->scan_add = scan_add;
                QByteArray fm_freq_data = readMemory(0x3400000, 0x40);
                fm->frequency = QString(fm_freq_data.mid(0, 0x4).toHex()).toInt();
                fm->name = QString(fm_freq_data.mid(4, 0x20));
                fm->name.remove(QChar('\0'));
            }
        }
    }
}
void Device::readRoamingChannelData(){
    QByteArray set_data = readMemory(Anytone::Memory::Map()->RoamingChannelSet, 0x20);
    std::vector<int> id_list;
    ...
    int offset = Anytone::Memory::Map()->RoamingChannelData;
    for(int i = 0; i < id_list.size(); i++){
        emit update2(i, id_list.size(), "Reading Roaming Channel Data");

        int idx = id_list.at(i);
        if(idx >= Anytone::Memory::roaming_channels.size()) continue;
        int addr = offset + (idx * Anytone::Memory::Map()->RoamingChannelDataOffset);
        QByteArray scan_list_data = readMemory(addr, Anytone::Memory::Map()->RoamingChannelDataLength);
        if(!is_alive) return;
        Anytone::Memory::roaming_channels.at(idx)->decode(scan_list_data);
    }
}
void Device::readRoamingZoneData(){
    QByteArray set_data = readMemory(Anytone::Memory::Map()->RoamingZoneSet, 0x10);
    std::vector<int> id_list;
    ...
    int offset = Anytone::Memory::Map()->RoamingZoneData;
    for(int i = 0; i < id_list.size(); i++){
        emit update2(i, id_list.size(), "Reading Roaming Zone Data");

        int idx = id_list.at(i);
        if(idx >= Anytone::Memory::roaming_zones.size()) continue;
        int addr = offset + (idx * Anytone::Memory::Map()->RoamingZoneDataOffset);
        QByteArray scan_list_data = readMemory(addr, Anytone::Memory::Map()->RoamingZoneDataLength);
        if(!is_alive) return;
...
void Device::readSettings(){
    ...
    if(Anytone::Memory::radio_model == Anytone::RadioModel::D878UVII_FW400){
        ...
        Anytone::Memory::optional_settings->decode_D878UVII(data_2500000, data_2500600, data_2501280, data_2501400);
        ...
        QByteArray data_3500000 = readMemory(0x3500000, 0x200);
        QByteArray data_3500900 = readMemory(0x3500900, 0x60);
        QByteArray data_3501280 = readMemory(0x3501280, 0x20);

        Anytone::Memory::optional_settings->decode_D890UV(data_3500000, data_3500900, data_3501280);
    }
}
...
void Device::readMasterId(){
    ...
    QByteArray master_radioid_data = readMemory(Anytone::Memory::Map()->MasterIdData, Anytone::Memory::Map()->MasterIdDataLength);
    Anytone::Memory::master_radio_id->decode(master_radioid_data);
}
void Device::readGpsRoamingData(){
    QByteArray gps_roaming_data = readMemory(Anytone::Memory::Map()->GpsRoamingData, Anytone::Memory::Map()->GpsRoamingDataLength);
...
void Device::readAutoRepeaterFrequencyData(){
    QByteArray auto_repeater_offset_data = readMemory(Anytone::Memory::Map()->AutoRepeaterData, 0x3f0);
    for(int i =0; i<250; i++){
...
void Device::readPrefabricatedSms(){
    ...
    while(current_idx <= 100){
        uint32_t addr = Anytone::Memory::Map()->PrefabSmsSet + (current_idx * 0x10);
    ...
    int offset = Anytone::Memory::Map()->PrefabSmsData;
    for(int i = 0; i < id_list.size(); i++){
        int idx = id_list.at(i);
        emit update2(i, id_list.size(), "Reading Prefabricated SMS");

        int block = int(((idx * Anytone::Memory::Map()->PrefabSmsDataOffset) - idx % Anytone::Memory::Map()->PrefabSmsDataBlockSize) / Anytone::Memory::Map()->PrefabSmsDataBlockSize);
        int addr = offset + (block * Anytone::Memory::Map()->PrefabSmsDataBlockOffset) + ((idx * Anytone::Memory::Map()->PrefabSmsDataOffset) % Anytone::Memory::Map()->PrefabSmsDataBlockSize);
        QByteArray sms_data = readMemory(addr, Anytone::Memory::Map()->PrefabSmsDataLength);
        Anytone::Memory::prefabricated_sms_list.at(idx)->decode(sms_data);
    }

}
void Device::readReceiveGroups(){
    QByteArray set_data = readMemory(Anytone::Memory::Map()->ReceiveGroupSet, 0x10);
    std::vector<int> id_list;
    ...
    int offset = Anytone::Memory::Map()->ReceiveGroupData;
    for(int i = 0; i < id_list.size(); i++){
        emit update2(i, id_list.size(), "Reading Receive Group Call List Data");

        int idx = id_list.at(i);
        int addr = offset + (idx * Anytone::Memory::Map()->ReceiveGroupDataOffset);
        Anytone::Memory::receive_group_call_lists.at(idx)->decode(readMemory(addr, Anytone::Memory::Map()->ReceiveGroupDataLength));

    }
}
void Device::readAesEncryptionKeys(){
    QByteArray set_data = readMemory(Anytone::Memory::Map()->AESSet, 0x10);
    std::vector<int> id_list;
    ...
    int offset = Anytone::Memory::Map()->AESData;
    for(int i = 0; i < id_list.size(); i++){
        emit update2(i, id_list.size(), "Reading AES Code Data");
        int addr = offset + (i*Anytone::Memory::Map()->AESDataOffset);
        Anytone::Memory::aes_encryption_keys.at(i)->decode(readMemory(addr, Anytone::Memory::Map()->AESDataLength));
    }
}
void Device::readArc4EncryptionKeys(){
    QByteArray set_data = readMemory(Anytone::Memory::Map()->Arc4Set, 0x10);
    std::vector<int> id_list;
    ...
    int offset = Anytone::Memory::Map()->Arc4Data;
    for(int i = 0; i < id_list.size(); i++){
        emit update2(i, id_list.size(), "Reading ARC4 Code Data");
        int idx = id_list.at(i);
        int addr = offset + (i * Anytone::Memory::Map()->Arc4DataOffset);
        Anytone::Memory::arc4_encryption_keys.at(i)->decode(readMemory(addr, Anytone::Memory::Map()->Arc4DataLength));
    }
}
...
void Device::writeChannelData(){
    int data_offset = Anytone::Memory::Map()->ChannelData;
    ...
    for(int i = 0; i < Anytone::Memory::channels.size(); i++){
        ...
        int channel_data_block = int(i / Anytone::Memory::Map()->ChannelDataBlockSize);
        int primary_data_address = data_offset + ((i - (channel_data_block * Anytone::Memory::Map()->ChannelDataBlockSize)) * Anytone::Memory::Map()->ChannelDataOffset) + (channel_data_block * Anytone::Memory::Map()->ChannelDataBlockOffset);
        int secondary_data_address = primary_data_address + Anytone::Memory::Map()->ChannelDataSecondaryOffset;

        QByteArray data = ch->encodeData();

        write_data[primary_data_address] = data.mid(0, 0x40);
        write_data[secondary_data_address] = data.mid(0x40, 0x40);
    }

    write_data[Anytone::Memory::Map()->ChannelSet] = set_list;
}
